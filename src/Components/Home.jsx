...
const slides = [
	{
		...
		image:
			"https://images.unsplash.com/photo-1528127269322-539801943592?w=1600&q=80&auto=format&fit=crop",
	...
	},
	{
		...
		image:
			"https://images.unsplash.com/photo-1524492412937-b28074a5d7da?w=1600&q=80&auto=format&fit=crop",
	...
	},
	{
		...
		image:
			"https://images.unsplash.com/photo-1559592413-7cec4d0cae2b?w=1600&q=80&auto=format&fit=crop",
	...
	},
	{
		...
		image:
			"https://images.unsplash.com/photo-1529253355930-ddbe423a2ac7?w=1600&q=80&auto=format&fit=crop",
...
const countryData = [
	{
		...
		facts: [
			...
			{ label: "Capital", value: "Hanoi" },
			{
				label: "Economy",
				value:
					"Rapidly growing manufacturing hub — strong in electronics, textiles, and agriculture",
			},
			{
				label: "Key Sectors",
				value: "Electronics, garments, seafood, agriculture, tourism",
			},
			{
				label: "Business Climate",
				value: "FDI-friendly, strategic ASEAN location, young workforce",
			},
		],
	},
	{
		...
		facts: [
			...
			{ label: "Capital", value: "New Delhi" },
			{
				label: "Economy",
				value:
					"World's fastest-growing major economy and IT services powerhouse",
			},
			{
				label: "Key Sectors",
				value:
					"IT services, pharmaceuticals, automotive, textiles, agriculture",
			},
			{
				label: "Business Climate",
				value:
					"Large domestic market, skilled workforce, growing digital economy",
			},
		],
	},
];
...
function AboutCard() {
	...
	return (
		<div ref={ref} className={`card-about ${visible ? "card-about-in" : ""}`}>
			<p className="text-gray-700 leading-relaxed text-lg">
				Vietnam India Services, Investment, and Trading is a premier bridge
				connecting the vibrant economies of Vietnam and India. We specialize in
				facilitating seamless business operations, investment opportunities, and
				trade relationships between these two rapidly growing markets.
			</p>
			<p className="text-gray-700 leading-relaxed text-lg">
				With deep understanding of both markets, regulatory environments, and
				cultural nuances, we provide comprehensive solutions for businesses
				looking to expand, invest, or trade across borders. Our mission is to
				empower companies with the tools, knowledge, and support they need to
				succeed.
			</p>
		</div>
	);
...
function Hero() {
	...
	function goTo(target) {
		...
		setTimeout(() => {
			setPrev(current);
			const next =
				target !== undefined ? target : (current + 1) % slides.length;
			setCurrent(next);
			setTextVisible(true);
			setTimeout(() => {
				setPrev(null);
	...
	return (
		<section className="hero-root">
			...
			{/* Dark + colour overlay */}
			<div
				className={`hero-overlay bg-gradient-to-r ${slide.accent} to-black/70`}
			/>
			...
			{/* ── Content ── */}
			<div
				className={`hero-content ${textVisible ? "hero-text-in" : "hero-text-out"}`}
			>
				{/* Country badge */}
				...
				<h1 className="text-5xl md:text-7xl font-extrabold text-white text-center leading-none mb-3 drop-shadow-xl">
					Vietnam <span className="hero-ampersand text-yellow-400">&amp;</span>{" "}
					India
				</h1>
			...
			<div className="hero-dots">
				{slides.map((s, i) => (
					...
					>
						<span
							className={`hero-dot-flag ${i === current ? "opacity-100" : "opacity-0"}`}
						>
							{s.flag}
						</span>
					</button>
		...
		<div className="space-y-14">
			...
			<section>
				...
				<div className="grid md:grid-cols-2 gap-8">
					{countryData.map(
						({ flag, name, headingClass, cardClass, facts }, idx) => (
							<CountryCard
								key={name}
								flag={flag}
								name={name}
